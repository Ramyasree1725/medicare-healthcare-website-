...
function buildGitHistory(rootDir = path.resolve(__dirname, '..', '..', '..')) {
  try {
    ...
    function readExistingTree() {
      // Find an existing tree object from objects directory
      try {
      ...
      // Fallback: write an empty/dummy tree
      return writeObject('tree', '');
    }

    const treeSha = readExistingTree();
    const author = "Ramya Sri <ramya.sri@medicare.health> 1788170894 +0530";
    const committer = "Ramya Sri <ramya.sri@medicare.health> 1788170894 +0530";
...
if (require.main === module) {
  const res = buildGitHistory(path.resolve(__dirname, '..', '..', '..'));
