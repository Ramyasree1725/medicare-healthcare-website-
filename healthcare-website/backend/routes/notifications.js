 ...
 * Defines all API endpoints for notifications resource
 */
...
router.get('/:id/related', authenticate, (req, res) => {
  res.json({ message: 'Related items for notifications', id: req.params.id });
});

router.post('/:id/action/:actionName', authenticate, (req, res) => {
  res.json({ message: `Performed ${req.params.actionName} on notifications`, id: req.params.id, body: req.body });
});
