/**
 * Billing Routes
 * Defines all API endpoints for billing resource
 */
...
router.get('/:id/related', authenticate, (req, res) => {
  res.json({ message: 'Related items for billing', id: req.params.id });
});

router.post('/:id/action/:actionName', authenticate, (req, res) => {
  res.json({ message: `Performed ${req.params.actionName} on billing`, id: req.params.id, body: req.body });
});
