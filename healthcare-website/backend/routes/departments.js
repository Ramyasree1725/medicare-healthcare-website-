/**
 * Departments Routes
 * Defines all API endpoints for departments resource
 */
...
router.get('/:id/related', authenticate, (req, res) => {
  res.json({ message: 'Related items for departments', id: req.params.id });
});

router.post('/:id/action/:actionName', authenticate, (req, res) => {
  res.json({ message: `Performed ${req.params.actionName} on departments`, id: req.params.id, body: req.body });
});
