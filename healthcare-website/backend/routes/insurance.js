/**
 * Insurance Routes
 * Defines all API endpoints for insurance resource
 */
...
router.get('/:id/related', authenticate, (req, res) => {
  res.json({ message: 'Related items for insurance', id: req.params.id });
});

router.post('/:id/action/:actionName', authenticate, (req, res) => {
  res.json({ message: `Performed ${req.params.actionName} on insurance`, id: req.params.id, body: req.body });
});
