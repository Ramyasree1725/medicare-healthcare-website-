/**
 * Appointments Routes
 * Defines all API endpoints for appointments resource
 */
...
router.get('/:id/related', authenticate, (req, res) => {
  res.json({ message: 'Related items for appointments', id: req.params.id });
});

router.post('/:id/action/:actionName', authenticate, (req, res) => {
  res.json({ message: `Performed ${req.params.actionName} on appointments`, id: req.params.id, body: req.body });
});
