/**
 * Pharmacy Routes
 * Defines all API endpoints for pharmacy resource
 */
...
router.get('/:id/related', authenticate, (req, res) => {
  res.json({ message: 'Related items for pharmacy', id: req.params.id });
});

router.post('/:id/action/:actionName', authenticate, (req, res) => {
  res.json({ message: `Performed ${req.params.actionName} on pharmacy`, id: req.params.id, body: req.body });
});
