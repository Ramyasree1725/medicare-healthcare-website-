/**
 * Doctors Routes
 * Defines all API endpoints for doctors resource
 */
...
router.get('/:id/related', authenticate, (req, res) => {
  res.json({ message: 'Related items for doctors', id: req.params.id });
});

router.post('/:id/action/:actionName', authenticate, (req, res) => {
  res.json({ message: `Performed ${req.params.actionName} on doctors`, id: req.params.id, body: req.body });
});
