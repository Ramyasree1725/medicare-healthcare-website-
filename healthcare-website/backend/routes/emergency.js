/**
 * Emergency Routes
 * Defines all API endpoints for emergency resource
 */
...
router.get('/:id/related', authenticate, (req, res) => {
  res.json({ message: 'Related items for emergency', id: req.params.id });
});

router.post('/:id/action/:actionName', authenticate, (req, res) => {
  res.json({ message: `Performed ${req.params.actionName} on emergency`, id: req.params.id, body: req.body });
});
