/**
 * Patients Routes
 * Defines all API endpoints for patients resource
 */
...
router.get('/:id/related', authenticate, (req, res) => {
  res.json({ message: 'Related items for patients', id: req.params.id });
});

router.post('/:id/action/:actionName', authenticate, (req, res) => {
  res.json({ message: `Performed ${req.params.actionName} on patients`, id: req.params.id, body: req.body });
});
