/**
 * Reports Routes
 * Defines all API endpoints for reports resource
 */
...
router.get('/:id/related', authenticate, (req, res) => {
  res.json({ message: 'Related items for reports', id: req.params.id });
});

router.post('/:id/action/:actionName', authenticate, (req, res) => {
  res.json({ message: `Performed ${req.params.actionName} on reports`, id: req.params.id, body: req.body });
});
