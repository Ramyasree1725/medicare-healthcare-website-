/**
 * Symptoms Routes
 * Defines all API endpoints for symptoms resource
 */
...
router.get('/:id/related', authenticate, (req, res) => {
  res.json({ message: 'Related items for symptoms', id: req.params.id });
});

router.post('/:id/action/:actionName', authenticate, (req, res) => {
  res.json({ message: `Performed ${req.params.actionName} on symptoms`, id: req.params.id, body: req.body });
});
