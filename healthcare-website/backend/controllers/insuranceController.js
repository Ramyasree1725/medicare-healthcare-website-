 ...
 * Handles all HTTP requests related to insurance
 */
...
exports.getAll = async (req, res) => {
  try {
    // getAll operation for insurance
    ...
    console.error('getAll error in insurance:', error);
...
exports.getById = async (req, res) => {
  try {
    // getById operation for insurance
    const { page = 1, limit = 20, sort = 'createdAt', order = 'desc' } = req.query;
    const skip = (parseInt(page) - 1) * parseInt(limit);
    ...
    console.error('getById error in insurance:', error);
    res.status(500).json({ error: 'Internal server error', message: error.message });
  }
};
...
exports.create = async (req, res) => {
  try {
    // create operation for insurance
    const { page = 1, limit = 20, sort = 'createdAt', order = 'desc' } = req.query;
    const skip = (parseInt(page) - 1) * parseInt(limit);
    ...
    console.error('create error in insurance:', error);
    res.status(500).json({ error: 'Internal server error', message: error.message });
  }
};
...
exports.update = async (req, res) => {
  try {
    // update operation for insurance
    const { page = 1, limit = 20, sort = 'createdAt', order = 'desc' } = req.query;
    const skip = (parseInt(page) - 1) * parseInt(limit);
    ...
    console.error('update error in insurance:', error);
    res.status(500).json({ error: 'Internal server error', message: error.message });
  }
};
...
exports.delete = async (req, res) => {
  try {
    // delete operation for insurance
    const { page = 1, limit = 20, sort = 'createdAt', order = 'desc' } = req.query;
    const skip = (parseInt(page) - 1) * parseInt(limit);
    ...
    console.error('delete error in insurance:', error);
    res.status(500).json({ error: 'Internal server error', message: error.message });
  }
};
...
exports.search = async (req, res) => {
  try {
    // search operation for insurance
    const { page = 1, limit = 20, sort = 'createdAt', order = 'desc' } = req.query;
    const skip = (parseInt(page) - 1) * parseInt(limit);
    ...
    console.error('search error in insurance:', error);
    res.status(500).json({ error: 'Internal server error', message: error.message });
  }
};
...
exports.filter = async (req, res) => {
  try {
    // filter operation for insurance
    const { page = 1, limit = 20, sort = 'createdAt', order = 'desc' } = req.query;
    const skip = (parseInt(page) - 1) * parseInt(limit);
    ...
    console.error('filter error in insurance:', error);
    res.status(500).json({ error: 'Internal server error', message: error.message });
  }
};
...
exports.export = async (req, res) => {
  try {
    // export operation for insurance
    const { page = 1, limit = 20, sort = 'createdAt', order = 'desc' } = req.query;
    const skip = (parseInt(page) - 1) * parseInt(limit);
    ...
    console.error('export error in insurance:', error);
    res.status(500).json({ error: 'Internal server error', message: error.message });
  }
};
...
exports.import = async (req, res) => {
  try {
    // import operation for insurance
    const { page = 1, limit = 20, sort = 'createdAt', order = 'desc' } = req.query;
    const skip = (parseInt(page) - 1) * parseInt(limit);
    ...
    console.error('import error in insurance:', error);
    res.status(500).json({ error: 'Internal server error', message: error.message });
  }
};
...
exports.stats = async (req, res) => {
  try {
    ...
    console.error('stats error in insurance:', error);
    res.status(500).json({ error: 'Internal server error', message: error.message });
  }
};

function processStep0(req, options) {
  // Helper processing function 0 for insurance controller
  try {
...
function processStep1(req, options) {
  // Helper processing function 1 for insurance controller
  try {
    const { page, limit, skip } = options;
    // Validation logic
    if (page < 1) return { error: 'Invalid page number' };
...
function processStep2(req, options) {
  // Helper processing function 2 for insurance controller
  try {
    const { page, limit, skip } = options;
    // Validation logic
    if (page < 1) return { error: 'Invalid page number' };
...
function processStep3(req, options) {
  // Helper processing function 3 for insurance controller
  try {
    const { page, limit, skip } = options;
    // Validation logic
    if (page < 1) return { error: 'Invalid page number' };
...
function processStep4(req, options) {
  // Helper processing function 4 for insurance controller
  try {
    const { page, limit, skip } = options;
    // Validation logic
    if (page < 1) return { error: 'Invalid page number' };
...
function processStep5(req, options) {
  // Helper processing function 5 for insurance controller
  try {
    const { page, limit, skip } = options;
    // Validation logic
    if (page < 1) return { error: 'Invalid page number' };
...
function processStep6(req, options) {
  // Helper processing function 6 for insurance controller
  try {
    const { page, limit, skip } = options;
    // Validation logic
    if (page < 1) return { error: 'Invalid page number' };
...
function processStep7(req, options) {
  // Helper processing function 7 for insurance controller
  try {
    const { page, limit, skip } = options;
    // Validation logic
    if (page < 1) return { error: 'Invalid page number' };
...
function processStep8(req, options) {
  // Helper processing function 8 for insurance controller
  try {
    const { page, limit, skip } = options;
    // Validation logic
    if (page < 1) return { error: 'Invalid page number' };
...
function processStep9(req, options) {
  // Helper processing function 9 for insurance controller
  try {
    const { page, limit, skip } = options;
    // Validation logic
    if (page < 1) return { error: 'Invalid page number' };
...
function processStep10(req, options) {
  // Helper processing function 10 for insurance controller
  try {
    const { page, limit, skip } = options;
    // Validation logic
    if (page < 1) return { error: 'Invalid page number' };
...
function processStep11(req, options) {
  // Helper processing function 11 for insurance controller
  try {
    const { page, limit, skip } = options;
    // Validation logic
    if (page < 1) return { error: 'Invalid page number' };
...
function processStep12(req, options) {
  // Helper processing function 12 for insurance controller
  try {
    const { page, limit, skip } = options;
    // Validation logic
    if (page < 1) return { error: 'Invalid page number' };
...
function processStep13(req, options) {
  // Helper processing function 13 for insurance controller
  try {
    const { page, limit, skip } = options;
    // Validation logic
    if (page < 1) return { error: 'Invalid page number' };
...
function processStep14(req, options) {
  // Helper processing function 14 for insurance controller
  try {
    const { page, limit, skip } = options;
    // Validation logic
    if (page < 1) return { error: 'Invalid page number' };
...
function processStep15(req, options) {
  // Helper processing function 15 for insurance controller
  try {
    const { page, limit, skip } = options;
    // Validation logic
    if (page < 1) return { error: 'Invalid page number' };
...
function processStep16(req, options) {
  // Helper processing function 16 for insurance controller
  try {
    const { page, limit, skip } = options;
    // Validation logic
    if (page < 1) return { error: 'Invalid page number' };
...
function processStep17(req, options) {
  // Helper processing function 17 for insurance controller
  try {
    const { page, limit, skip } = options;
    // Validation logic
    if (page < 1) return { error: 'Invalid page number' };
...
function processStep18(req, options) {
  // Helper processing function 18 for insurance controller
  try {
    const { page, limit, skip } = options;
    // Validation logic
    if (page < 1) return { error: 'Invalid page number' };
...
function processStep19(req, options) {
  // Helper processing function 19 for insurance controller
  try {
    const { page, limit, skip } = options;
    // Validation logic
    if (page < 1) return { error: 'Invalid page number' };
...
function processStep20(req, options) {
  // Helper processing function 20 for insurance controller
  try {
    const { page, limit, skip } = options;
    // Validation logic
    if (page < 1) return { error: 'Invalid page number' };
...
function processStep21(req, options) {
  // Helper processing function 21 for insurance controller
  try {
    const { page, limit, skip } = options;
    // Validation logic
    if (page < 1) return { error: 'Invalid page number' };
...
function processStep22(req, options) {
  // Helper processing function 22 for insurance controller
  try {
    const { page, limit, skip } = options;
    // Validation logic
    if (page < 1) return { error: 'Invalid page number' };
...
function processStep23(req, options) {
  // Helper processing function 23 for insurance controller
  try {
    const { page, limit, skip } = options;
    // Validation logic
    if (page < 1) return { error: 'Invalid page number' };
...
function processStep24(req, options) {
  // Helper processing function 24 for insurance controller
  try {
    const { page, limit, skip } = options;
    // Validation logic
    if (page < 1) return { error: 'Invalid page number' };
...
function processStep25(req, options) {
  // Helper processing function 25 for insurance controller
  try {
    const { page, limit, skip } = options;
    // Validation logic
    if (page < 1) return { error: 'Invalid page number' };
...
function processStep26(req, options) {
  // Helper processing function 26 for insurance controller
  try {
    const { page, limit, skip } = options;
    // Validation logic
    if (page < 1) return { error: 'Invalid page number' };
...
function processStep27(req, options) {
  // Helper processing function 27 for insurance controller
  try {
    const { page, limit, skip } = options;
    // Validation logic
    if (page < 1) return { error: 'Invalid page number' };
...
function processStep28(req, options) {
  // Helper processing function 28 for insurance controller
  try {
    const { page, limit, skip } = options;
    // Validation logic
    if (page < 1) return { error: 'Invalid page number' };
...
function processStep29(req, options) {
  // Helper processing function 29 for insurance controller
  try {
    const { page, limit, skip } = options;
    // Validation logic
    if (page < 1) return { error: 'Invalid page number' };
